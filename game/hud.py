...
class Hud:
    ...
    def update(self):
        ...
        # If the mouse right clicks
        if mouse_action[2]:
            self.selected_tile = None

        # Check if a tile is selected
        for tile in self.tiles:
            # If the rect has collided with a single point, that is our mouse
            if tile['rect'].collidepoint(mouse_pos):
                # If we left click, is the first index of mouse action
                if mouse_action[0]:
